...
$passID = (isset($_GET['add']) ? $_GET['add'] : null);
...
function products() {
	...
	$price=$_GET['price'];	
		...
		include './includes/connection.php';
		 ...
		 $query ="SELECT * FROM product WHERE name LIKE '%$search%' OR description LIKE '%$search%'  order by category";
...
.'<img src="images/dbImages/'.$row['imagename'].'" height="200" width="250">' 
  ...
  .'<td style="width:50px" >'
  .'&pound'. $row["shipping"]
.'<td style="width:50px">'.'<a href="cart.php?add=' . $row['id'] . '">Add To Cart</a>'
.'</td>'.'</table>';}
...
<p>
	<form action="https://www.paypal.com/cgi-bin/webscr" method="post">
		...
		<input type="hidden" name="business" value="joy@gmail.com">
		<?php paypal_items(); ?>
